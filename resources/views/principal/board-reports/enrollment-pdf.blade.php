...
<body>
    ...
    <div class="title">Enrollment Summary — {{ now()->format('F Y') }}</div>
    <table>
        <thead><tr><th>Department</th><th>Students</th><th>New this year</th></tr></thead>
        <tbody>
            @foreach ($enrollment as $row)
                <tr><td>{{ $row->department }}</td><td>{{ $row->total }}</td><td>+{{ $row->newThisYear }}</td></tr>
            @endforeach
